...
import utils
from dataset import create_dataset_vilt, create_sampler, create_loader
...
from models.Vilt import HAMMER
# from models.my_model import My_HAMMER
...
def text_input_adjust(text_input, fake_word_pos, device):
    ...
    # text_input.pixel_values = torch.LongTensor(text_input.pixel_values).to(device)
...
def train(args, model, data_loader, optimizer, tokenizer, epoch, warmup_steps, device, scheduler, config, summary_writer):
    ...
    processor = ViltProcessor.from_pretrained("./DGM4/vilt_b32_mlm")

    if args.distributed:
        data_loader.sampler.set_epoch(epoch)

    for i, (image, caption, label, fake_image_box, fake_word_pos, W, H) in enumerate(metric_logger.log_every(args, data_loader, print_freq, header)):
        ...
        image = processor.feature_extractor(images=image, return_tensors="pt", do_rescale=False)
        image = image.to(device)
        text_inputs = processor.tokenizer(text = caption, max_length=40, truncation=True, add_special_tokens=True, return_attention_mask=True, return_token_type_ids=False) 
        text_inputs, fake_token_pos = text_input_adjust(text_inputs, fake_word_pos, device)
        ...
        loss_BIC, loss_bbox, loss_giou, loss_TMG, loss_MLC = model(image, text_inputs, label,fake_image_box, fake_token_pos, alpha = alpha)  
...
def evaluation(args, model, data_loader, tokenizer, device, config):
    ...
    for i, (image, text, label, fake_image_box, fake_word_pos, W, H) in enumerate(metric_logger.log_every(args, data_loader, print_freq, header)):
        
        image = image.to(device,non_blocking=True) 
        image = processor.feature_extractor(images=image, return_tensors="pt", do_rescale=False)
        image = image.to(device)
        text_inputs = processor.tokenizer(text = text, max_length=40, truncation=True, add_special_tokens=True, return_attention_mask=True, return_token_type_ids=False) 
        text_inputs, fake_token_pos = text_input_adjust(text_inputs, fake_word_pos, device)

        logits_real_fake, logits_multicls, output_coord, logits_tok = model(image, text_inputs, label, fake_image_box, fake_token_pos, is_train=False)
        ...
        # ----- multi metrics -----
        target, _ = get_multi_label(label, image.pixel_values)
        ...
        ##================= token cls ========================##  
        token_label = text_inputs.attention_mask[:,1:].clone() # [:,1:] for ingoring class token
        ...
        logits_tok_reshape = logits_tok.reshape(-1, 2)
...
def main_worker(gpu, args, config):
    ...
    train_dataset, val_dataset = create_dataset_vilt(config)
    ...
    for epoch in range(start_epoch, max_epoch):
        ...
        if utils.is_main_process(): 
            ...
            current_auc = float(val_stats['AUC_cls'])
            if current_auc > best:
            ...
            if early_stop_patience > 0 and early_stop_bad_epochs >= early_stop_patience:
                stop_training = True
        ...
        if stop_tensor.item() == 1:
            if utils.is_main_process() and args.log:
                print(
                    ...
                    f"best AUC_cls={best:.4f} at epoch {best_epoch}",
                    flush=True,
                )
            break
