...
from models.clip import HAMMER
...
def text_input_adjust(text_input, fake_word_pos, device):
    ...
    text_input_ = copy.deepcopy(text_input)
    ...
    for i in range(len(fake_word_pos)): # fake_word_pos 32,50
        fake_token_pos = []
        ...
        # subword_idx = text_input.word_ids(i)
        length = len(text_input_.input_ids[i]) - 2
        subword_idx_rm_CLSSEP = list(range(length))
...
def train(args, model, data_loader, optimizer, tokenizer, epoch, warmup_steps, device, scheduler, config, summary_writer):
    ...
    for i, (image, label, text, fake_image_box, fake_word_pos, W, H) in enumerate(metric_logger.log_every(args, data_loader, print_freq, header)):
        ...
        text_input = tokenizer(text, max_length=49,padding='max_length',truncation=True, add_special_tokens=True, return_attention_mask=True, return_token_type_ids=False) 
        ...
        loss_BIC, loss_bbox, loss_giou, loss_TMG, loss_MLC = model(image, label, text, text_input, fake_image_box, fake_token_pos, alpha = alpha)  
...
def evaluation(args, model, data_loader, tokenizer, device, config):
    ...
    for i, (image, label, text, fake_image_box, fake_word_pos, W, H) in enumerate(metric_logger.log_every(args, data_loader, print_freq, header)):
        ...
        logits_real_fake, logits_multicls, output_coord, logits_tok = model(image, label, text, text_input, fake_image_box, fake_token_pos, is_train=False)
...
def main_worker(gpu, args, config):
    ...
    tokenizer = CLIPProcessor.from_pretrained("./datasets/clip-vit-large-patch14/")
    ...
    for epoch in range(start_epoch, max_epoch):
        ...
        if stop_tensor.item() == 1:
            if args.log and utils.is_main_process():
                print(f"Early stopping at epoch {epoch}, best epoch {best_epoch} with AUC_cls {best:.4f}")
            break
